...
class QvTaulaQuery(QtWidgets.QWidget):
    ...
    def pinta(self,index):
        ...
        print(self.model2.data(index))
        print(self.model2.record(index.row()).fieldName(index.column()))
        print(self.model2.record(index.row()).value("qv_X"), self.model2.record(index.row()).value("qv_Y"))
        x = float(self.model2.record(index.row()).value("qv_X"))
        y = float(self.model2.record(index.row()).value("qv_Y"))
        rang = QgsRectangle(x-50, y-50, x+50, y+50)
...
if __name__ == "__main__":
    with qgisapp():
        ...
        db.setDatabaseName('sports60.db')
        ...
        query.exec_("insert into sportsmen values(1023, 'fChristiano', 'Pernaldo')")
