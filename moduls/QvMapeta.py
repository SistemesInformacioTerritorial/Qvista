...
class QvMapeta(QFrame):
    ...
    def __init__(self, canvas, tamanyPetit = False, pare = None):
        ...
        if self.tamanyPetit:
            self.xTamany = 179
            self.yTamany = 180
            self.setStyleSheet('QFrame {opacity: 50; background-image: url("../dades/imatges/MapetaPetit.jpg");}')
        else:
            self.xTamany = 267
            self.yTamany = 284
            self.setStyleSheet('QFrame {background-image: url("../dades/imatges/MAPETA_SITUACIO_267x284.bmp");}')
        ...
        icon = QIcon('../dades/imatges/mapeta-collapse.png')
    ...
    def ferPetit(self):
        if self.petit:
            self.setGeometry(0,0,self.xTamany,self.yTamany)
            self.petit = False
            icon = QIcon('../dades/imatges/arrow-collapse.png')
            self.botoFerPetit.setIcon(icon)
        else:
            self.setGeometry(0,0,25,25)
            self.petit = True
            icon = QIcon('../dades/imatges/mapetaPetit.jpg')
            self.botoFerPetit.setIcon(icon)
