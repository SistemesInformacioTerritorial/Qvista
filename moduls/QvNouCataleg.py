...
class QvNouCataleg(QWidget):
    ...
    def __init__(self, parent: QtWidgets.QWidget = None):
        ...
        self.widgetSup.setStyleSheet('color: %s; background-color: solid %s; border: 0px;' %
        ...
        self.spacer = QtWidgets.QSpacerItem(
            99999, 99999, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
    ...
    def clickFavorits(self):
        ...
        for x in self.botonsLaterals:
            ...
            lbl = QLabel(x.text())
            lbl.setStyleSheet('background: %s; padding: 2px;' %
                              QvConstants.COLORGRISHTML)
    ...
    def obrirInfo(self, dir: str):
        ...
        if os.path.exists(dir):
            ...
            QMessageBox.warning(self, "No s'ha trobat la informació",
                                "La informació del mapa no ha pogut ser oberta. Si el problema persisteix, contacteu amb el gestor del catàleg")

    def mousePressEvent(self, event):
        # La capçalera té mida 40. Si fem click a la capçalera, podem moure
...
class MapaCataleg(QFrame):
    ...
    def setChecked(self, checked: bool):
        ...
        if checked:
            self.setStyleSheet(
                'MapaCataleg{border: 4px solid %s;}' % QvConstants.COLORCLARHTML)
            self.botoFav.show()
            self.botoObre.show()
            self.botoInfo.show()
            self.botoQGis.show()
...
class QvCreadorCataleg(QDialog):
    ...
    def _cbCatalegsCanviat(self, i):
        catalegsLocals = [os.path.abspath(x).lower()
                          for x in QvMemoria().getCatalegsLocals()]
        if not os.path.exists(self._cbCatalegs.currentText()):
            ret = QFileDialog.getExistingDirectory(
                self, 'Trieu la carpeta del catàleg', QvMemoria().getDirectoriDesar())
            ret = os.path.abspath(ret).lower()
            if ret == '':
                self._cbCatalegs.setCurrentIndex(0)
                return
            if ret in carpetaCatalegProjectesLlista:
                r = QMessageBox.question(self, 'Heu triat un catàleg compartit',
                                         'La carpeta que heu triat és un catàleg compartit. Esteu segur que voleu i podeu modificar-lo?', QMessageBox.Yes, QMessageBox.No)
                if r == QMessageBox.No:
                    self._cbCatalegs.setCurrentIndex(0)
                    return
                ...
                resp = QMessageBox.question(
                    self, 'Afegir el catàleg', 'Voleu convertir aquesta carpeta en un catàleg de mapes local?', QMessageBox.Yes, QMessageBox.No)
                ...
                self._cbCatalegs.setCurrentIndex(catalegsLocals.index(ret))
    ...
    def _desar(self):
        ...
        # Obtenim la carpeta on desarem
        if self._leNovaCat.isVisible():
            cat = self._leNovaCat.text()
            ret = os.path.join(self._cbCatalegs.currentText(), cat)
            os.mkdir(ret)
        else:
            ret = os.path.join(self._cbCatalegs.currentText(),
                               self._cbSubcarpetes.currentText())

        # Amb el nom del projecte i la carpeta obtenim la ruta completa on volem desar, sense extensió
