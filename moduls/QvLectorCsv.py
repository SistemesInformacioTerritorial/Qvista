...
class QvLectorCsv(QtWidgets.QWidget):
    def __init__(self, fileName="", separador=None, completa=False, parent=None):
        ...
        self.carregaCsv(self.fileName, separador, completa)
        self.separador = separador
        self.setMinimumHeight(200)
        if completa:
            # Hem d'afegir un botó de desar
            desar = QtWidgets.QPushButton('Desar csv')
            grid.addWidget(desar, 2, 0)
            desar.clicked.connect(lambda: self.writeCsv('qVista.csv'))
...
#            print(mytext)
    ...
    def carregaCsv(self, fileName, separador, completa=False):
        ...
        if fileName:
            ...
            with f:
                ...
                for row in reader:
                    ...
                    if (i == 0):
                        self.capcaleres = [x.text() for x in items]
                        self.model.setHorizontalHeaderLabels(self.capcaleres)
                    else:
                        self.model.appendRow(items)
                    i += 1
                self.tableView.resizeColumnsToContents()
    ...
    def writeCsv(self, fileName):
        ...
        if fileName:
            ...
            with f:
                ...
                writer.writerow(self.capcaleres)
