...
class QvCarregaCsv(QDialog):
    ...
    def triaPrimeraPantalla(self):
        campX, campY = campsCoords(self._csv, self._separador, self._codificacio)
...
class CsvPagina(QWidget):
    ...
    def _mostraTaula(self, errors=[]):
        ...
        wid = QvEditorCsv(self.parentWidget()._csv, errors, self.parentWidget()._codificacio, self.parentWidget()._separador)
    ...
    def _mapifica(self):
        fMap = QvFormNovaMapificacio(self.parentWidget()._qV.llegenda, mapificacio=self.parentWidget()._mapificador)
        fMap.exec()
...
class CsvCoords(CsvPagina):
    def __init__(self, parent=None, campX=None, campY=None):
        ...
        self._lay.addWidget(QLabel("Introduiu els camps que defineixen les coordenades"))
    ...
    def afegir(self):
        self.salta.emit(CsvAfegir(self._cbX.currentText(),self._cbY.currentText(),self._proj[self._cbProj.currentText()],self.parentWidget()))
...
class CsvAdreca(CsvPagina):
    def __init__(self, parent=None, **camps):
        ...
        self._lay.addWidget(QLabel("Tria dels camps que defineixen l'adreça.\nNomés són obligatoris el nom de la via i el número inicial"))
...
class CsvGeocod(CsvPagina):
    ...
    def showEvent(self,e):
        ...
        self.parentWidget()._mapificador.geocodificacio(self._camps, ('Coordenada', 'Districte', 'Barri', 'Codi postal', "Illa", "Solar", "Àrea estadística bàsica", "Secció censal"), percentatgeProces=lambda x: self._progress.setValue(x), errorAdreca=self._unErrorMes, procesAcabat=self.acabat)
...
class CsvGeocodificat(CsvPagina):
    def __init__(self,errors, parent=None):
        ...
        self._bVeure.clicked.connect(self._mostraTaula)
        self._bAfegir=QvPushButton('Afegir')
        self._bAfegir.clicked.connect(lambda: self.salta.emit(CsvAfegir('QVISTA_ETRS89_COORD_X','QVISTA_ETRS89_COORD_Y',25831,self.parentWidget())))
    ...
    def _definirErrors(self,errors):
        ...
        for x in errors:
            errorsStr=errorsStr+'\nFila %i'%x
        self._textEditErrors.setText('Files amb errors (click sobre "Veure" per veure i editar la taula):%s'%errorsStr)
        pass
...
class CsvAfegir(CsvPagina):
    def __init__(self,campCoordX, campCoordY, projeccio, parent=None):
        ...
        def setColorBoto():
            self._bColor.setStyleSheet('color: %s; background-color: %s'%(self._color,self._color))
        def obrirDialegColor():
            color=QColorDialog.getColor(QColor(self._color), self,'Tria del color de la representació')
        ...
        self._bAfegir.clicked.connect(self.afegir)
        self._leNomCapa.textChanged.connect(lambda x: self._bAfegir.setEnabled(x!=''))
        layBotons.addWidget(self._bAfegir)
        self._lay.addLayout(layBotons)

    def afegir(self):
        nivellCsv(self.parentWidget()._qV, self.parentWidget()._csv,self.parentWidget()._separador, self._campCoordX, self._campCoordY, self._projeccio, self._leNomCapa.text(),self._color,self._forma)
        self.parentWidget().close()
