...
def carregarLayerCSV(nfile):
        if nfile: 
            with open(nfile) as arxiuCsv:
                    ...
                    global tamanyReader
                    tamanyReader=len(arxiuCsv.readlines())-1
                    varX=''
                    varY=''
                    
                    def actualitzaCamps():
                            global separador, reader, globalLlistaCamps
                            #global cbAdreca, cbViaCarrer, cbNumVC, cbVia
                            if radio1.isChecked():
                                    separador=';'
                            ...
                            cbAdreca.addItems(llistaCamps)
                            cbViaCarrer.addItems(llistaCamps)
                            cbNumVC.addItems(llistaCamps)
                            cbVia.addItems(llistaCamps)
                            cbNomVia.addItems(llistaCamps)
                            cbNum.addItems(llistaCamps)

                            table.recarrega(separador)
                            globalLlistaCamps=llistaCamps.copy()
                    ...
                    layRadioB = QHBoxLayout()
                    rbXiY = QRadioButton("Amb camps X i Y")
                    rbAdreca = QRadioButton("Amb l'adreça")
                    layRadioB.addWidget(rbXiY)
                    layRadioB.addWidget(rbAdreca)
                    rbXiY.toggled.connect(lambda: acceptar.setEnabled(True))
                    rbAdreca.toggled.connect(lambda: acceptar.setEnabled(True))

                    layXY = QVBoxLayout()
                    layXY.addLayout(layCbx)
                    layXY.addLayout(layCby)
                    layXY.addLayout(layProj)

                    boxXiY = QGroupBox("")
                    boxXiY.setLayout(layXY)

                    tabs = QTabWidget()

                    tab1 = QWidget()
                    layAdreca1 = QVBoxLayout()
                    layContAdreca = QHBoxLayout()
                    lblAdreca = QLabel()
                    lblAdreca.setText("Adreça")
                    layContAdreca.addWidget(lblAdreca)
                    cbAdreca=QComboBox()
                    layContAdreca.addWidget(cbAdreca)
                    lblBlanc1 = QLabel()
                    lblBlanc1.setText("")
                    lblBlanc2 = QLabel()
                    lblBlanc2.setText("")
                    layAdreca1.addLayout(layContAdreca)
                    layAdreca1.addWidget(lblBlanc1)
                    layAdreca1.addWidget(lblBlanc2)
                    tab1.setLayout(layAdreca1)
                    tabs.addTab(tab1,"1")

                    tab2 = QWidget()
                    layAdreca2 = QVBoxLayout()
                    layA2E1 = QHBoxLayout()
                    lblViaCarrer = QLabel()
                    lblViaCarrer.setText("Carrer")
                    layA2E1.addWidget(lblViaCarrer)
                    cbViaCarrer=QComboBox()
                    layA2E1.addWidget(cbViaCarrer)
                    layA2E2 = QHBoxLayout()
                    lblNumVC = QLabel()
                    lblNumVC.setText("Número")
                    layA2E2.addWidget(lblNumVC)
                    cbNumVC=QComboBox()
                    layA2E2.addWidget(cbNumVC)
                    lblBlanc3 = QLabel()
                    lblBlanc3.setText("")
                    layAdreca2.addLayout(layA2E1)
                    layAdreca2.addLayout(layA2E2)
                    layAdreca2.addWidget(lblBlanc3)
                    tab2.setLayout(layAdreca2)
                    tabs.addTab(tab2,"2")

                    tab3 = QWidget()
                    layAdreca3 = QVBoxLayout()
                    layA3E1 = QHBoxLayout()
                    lblVia = QLabel()
                    lblVia.setText("Tipus Via")
                    layA3E1.addWidget(lblVia)
                    cbVia=QComboBox()
                    layA3E1.addWidget(cbVia)
                    layA3E2 = QHBoxLayout()
                    lblCarrer = QLabel()
                    lblCarrer.setText("Nom Via")
                    layA3E2.addWidget(lblCarrer)
                    cbNomVia=QComboBox()
                    layA3E2.addWidget(cbNomVia)
                    layA3E3 = QHBoxLayout()
                    lblNum = QLabel()
                    lblNum.setText("Número")
                    layA3E3.addWidget(lblNum)
                    cbNum=QComboBox()
                    layA3E3.addWidget(cbNum)
                    layAdreca3.addLayout(layA3E1)
                    layAdreca3.addLayout(layA3E2)
                    layAdreca3.addLayout(layA3E3)
                    tab3.setLayout(layAdreca3)
                    tabs.addTab(tab3,"3")


                    layAdreca = QVBoxLayout()
                    layAdreca.addWidget(tabs)

                    boxAdreca = QGroupBox("")
                    boxAdreca.setLayout(layAdreca)

                    layAuxContingut = QHBoxLayout()
                    layAuxContingut.addWidget(boxXiY)
                    layAuxContingut.addWidget(boxAdreca)
                    
                    layGeometry.addLayout(layRadioB)
                    layGeometry.addLayout(layAuxContingut)
                    ...
                    acceptar=QvPushButton('Acceptar', destacat=True)
        ...
        if rbXiY.isChecked() and radiob2.isChecked():
        ...
        elif rbXiY.isChecked():
                varX=cbx.currentText()
                varY=cby.currentText()
                projeccio = int(cbProj.currentText())
                import tempfile
                reader=csv.DictReader(open(nfile),delimiter=separador)
                with tempfile.NamedTemporaryFile(suffix='.csv',mode='w+',delete=False) as arxiuNouCsv:
                        writer=csv.DictWriter(arxiuNouCsv,fieldnames=globalLlistaCamps,delimiter=separador)
                        writer.writeheader()
                        for row in reader:
                                # d={x:y.replace(',','.') for x, y in row.items()}
                                d={**row}
                                d[varX]=d[varX].replace(',','.')
                                d[varY]=d[varY].replace(',','.')
                                writer.writerow(d)
                                print(d)
                        nom=leNom.text()
                ...
                with tempfile.NamedTemporaryFile(suffix='.csv', mode='w+',delete=False) as arxiuNouCsv:
                        ...
                        windowProgressBar = QWidget()
                        #windowProgressBar.setTitle("Progrés càlcul coordenades") 
                        progress = QProgressBar()
                        progress.setGeometry(0, 0, 300, 25)
                        progress.setMaximum(tamanyReader)
                        count = 0 #necessari per a la progress bar
                        progress.setValue(count)
                        layProgressW = QVBoxLayout()
                        layProgressW.addWidget(progress)
                        lblAdrInfo = QLabel()
                        lblAdrInfo.setText("Adreces que no s'han pogut geolocalitzar:")
                        layProgressW.addWidget(lblAdrInfo)
                        lblAdrecesError = QLabel()
                        lblAdrecesError.setText("")
                        lblAdrecesError.setStyleSheet('color: red')
                        lblAdrecesError.setFixedSize(300,50)
                        lblAdrecesError.setWordWrap(True)
                        layProgressW.addWidget(lblAdrecesError)
                        windowProgressBar.setLayout(layProgressW)
                        windowProgressBar.show()

                        #nomTemporal='%Temp%/CsvFile.csv'
                        #reader=DictReader(arxiuCsv,delimiter=separador)
                        names=globalLlistaCamps+['XCalculada','YCalculada']
                        writer=csv.DictWriter(arxiuNouCsv,fieldnames=names, delimiter=separador)
                        writer.writeheader()
                        ...
                        for row in reader:
                                ...
                                if tabs.currentIndex()==0:
                                        #print(cbAdreca.currentText())
                                        tipusVia, nomVia, num = splitCarrer(row[cbAdreca.currentText()])
                                else:
                                        # cbVia.clear()
                                        # cbNomVia.clear()
                                        # cbNum.clear()
                                        tipusVia=row[cbVia.currentText()]
                                        nomVia=row[cbNomVia.currentText()]
                                        num=row[cbNum.currentText()]
                                ...
                                if x is None or y is None:
                                        aux = lblAdrecesError.text()
                                        lblAdrecesError.setText(aux + "\n"+ tipusVia + " " + nomVia + " " + num)
